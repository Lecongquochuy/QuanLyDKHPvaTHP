...
{
    public partial class fAddSubjectOfUpdateOpenSubject : Form
    {
        public DataTable SubjectTable = new DataTable();
        public bool check = true;
        private bool add = true;
        private fUpdateOpenSubject fUpdateOpenSub;
        public fAddSubjectOfUpdateOpenSubject(fUpdateOpenSubject fUOS)
        {
            InitializeComponent();
        ...
        void LoadSubjectID()
        {
            string query = "SELECT MaMH " +
                ...
                "WHERE mh.MaMH in (SELECT MaMH FROM dbo.CT_NGANH as ct WHERE ct.MaNH = '" + fUpdateOpenSub.manganh + "') ";
            DataTable data = DataProvider.Instance.ExecuteQuery(query);
            foreach (DataRow row in data.Rows)
            {
                if (fUpdateOpenSub.CheckIfExists(row["MaMH"].ToString()))
                {
                    row.Delete();
                }
            }
        ...
        private void fAddSubjectOpenSubject_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (add)
            {
                DialogResult result = MessageBox.Show("Bạn có muốn lưu thông tin?", "Thông báo", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
