...
{
    public partial class fAddSubjectOfUpdateStudyProgram : Form
    {
        public DataTable SubjectTable = new DataTable();

        private fUpdateStudyProgram fUSP;
        public bool check = true;
        private bool add = true;

        public fAddSubjectOfUpdateStudyProgram(fUpdateStudyProgram check)
        {
            InitializeComponent();
            ...
            SubjectTable.Columns.Add("Số tín chỉ", typeof(string));
            fUSP = check;
            LoadSubjectID();
        ...
        void LoadSubjectID()
        {
            ...
            {
                if (fUSP.CheckIfExists(row["MaMH"].ToString()))
                {
                    row.Delete();
                }
            }
        ...
        public void AddSubject(object sender, EventArgs e)
        {
            ...
            {
                string SubjectIDValue = data.Rows[0]["MaMH"].ToString();
                bool exists = fUSP.CheckIfExists(SubjectIDValue);
                if (exists == false)
                {
        ...
        private void fAddSubjectOfUpdateStudyProgram_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (add)
            {
                if (check == true)
